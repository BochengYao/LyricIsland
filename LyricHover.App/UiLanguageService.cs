    ...
    public static class UiLanguageService
    {
        private static readonly Dictionary<string, string[]> Text = new Dictionary<string, string[]>
        {
            ...
            ["通用"] = new[] { "通用", "一般", "General", "一般" },
                        ...
                        ["了解节能模式"] = new[] { "了解节能模式", "了解省電模式", "About Power Saving", "省電力モードについて" },
            ...
            ["全屏时自动隐藏"] = new[] { "全屏时自动隐藏", "全螢幕時自動隱藏", "Hide in full screen", "全画面時に自動で隠す" },
            ["当前台应用覆盖歌词岛所在显示器时收起，退出全屏后恢复。"] = new[] { "当前台应用覆盖歌词岛所在显示器时收起，退出全屏后恢复。", "目前景應用程式覆蓋歌詞島所在顯示器時收起，離開全螢幕後恢復。", "Hides when the foreground app covers the lyrics island's monitor, then restores after full screen ends.", "前面のアプリが歌詞島のあるモニターを覆うと隠れ、全画面表示を終了すると復元します。" },
            ...
            ["收起"] = new[] { "收起", "收起", "Hide", "閉じる" },
            ["收起详情"] = new[] { "收起详情", "收起詳情", "Hide details", "詳細を隠す" },
            ["降低能耗，减少不必要的动态效果。"] = new[] { "降低能耗，减少不必要的动态效果。", "降低耗電，減少不必要的動態效果。", "Uses less power by reducing nonessential motion.", "不要な動きを抑え、消費電力を抑えます。" },
            ["播放时每秒更新一次；空闲时每四秒检查一次。"] = new[] { "播放时每秒更新一次；空闲时每四秒检查一次。", "播放時每秒更新一次；閒置時每四秒檢查一次。", "Updates once per second while playing, and checks every four seconds when idle.", "再生中は1秒ごとに更新し、待機中は4秒ごとに確認します。" },
            ["设置使用实体背景，并减少阴影、预览和过渡动画。"] = new[] { "设置使用实体背景，并减少阴影、预览和过渡动画。", "設定使用實體背景，並減少陰影、預覽與過渡動畫。", "Settings use a solid background with fewer shadows, previews, and transitions.", "設定画面は不透明な背景になり、影、プレビュー、トランジションを減らします。" },
            ["界面会更安静。歌词切换和悬停将直接响应。"] = new[] { "界面会更安静。歌词切换和悬停将直接响应。", "介面會更安靜。歌詞切換與懸停將直接回應。", "The interface stays quiet. Lyrics and hover states respond directly.", "表示はより静かになります。歌詞とホバーはすぐに反応します。" },
