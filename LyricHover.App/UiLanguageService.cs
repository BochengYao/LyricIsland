...
namespace LyricHover.App
{
    ...
    public static class UiLanguageService
    {
        private static readonly Dictionary<string, string[]> Text = new Dictionary<string, string[]>
        {
            ...
            ["通用"] = new[] { "通用", "一般", "General", "一般" },
                        ["节能模式"] = new[] { "节能模式", "省電模式", "Power saving mode", "省電力モード" },
                        ["完成"] = new[] { "完成", "完成", "Done", "完了" },
                        ["查看详情"] = new[] { "查看详情", "檢視詳情", "View details", "詳細を表示" },
                        ["用于保存已下载歌词，提高播放切换速度"] = new[] { "用于保存已下载歌词，提高播放切换速度", "用於儲存已下載歌詞，提高播放切換速度", "Stores downloaded lyrics to speed up playback switching", "ダウンロード済みの歌詞を保存し、曲の切り替えを高速化します" },
            ["歌词外观"] = new[] { "歌词外观", "歌詞外觀", "Lyrics appearance", "歌詞の外観" },
            ["歌词岛"] = new[] { "歌词岛", "歌詞島", "Lyrics island", "歌詞アイランド" },
            ...
            ["支持"] = new[] { "支持", "支持", "Support", "サポート" },
            ["收起详情"] = new[] { "收起详情", "收起詳情", "Hide details", "詳細を隠す" },
            ["降低刷新频率并关闭过渡动画，显著降低CPU与GPU占用，但歌词岛会失去流畅感"] = new[] { "降低刷新频率并关闭过渡动画，显著降低CPU与GPU占用，但歌词岛会失去流畅感", "降低重新整理頻率並關閉過渡動畫，顯著降低CPU與GPU占用，但歌詞島會失去流暢感", "Lowers refresh rates and disables transitions to greatly reduce CPU and GPU usage, but the lyrics island loses its smoothness", "更新頻度を下げ、トランジションを無効にしてCPUとGPUの使用量を大幅に減らしますが、歌詞アイランドの滑らかさは失われます" },
            ["定时任务降频：歌词岛主刷新从每秒4次降至每秒1次，鼠标接近探测从每秒25次降至每秒2次"] = new[] { "定时任务降频：歌词岛主刷新从每秒4次降至每秒1次，鼠标接近探测从每秒25次降至每秒2次", "定時任務降頻：歌詞島主重新整理從每秒4次降至每秒1次，滑鼠接近偵測從每秒25次降至每秒2次", "Scheduled tasks slow down: the main lyrics island refresh drops from 4 times per second to 1, and mouse proximity detection drops from 25 times per second to 2", "定期タスクの間引き：歌詞アイランドのメイン更新は毎秒4回から1回に、マウス近接検知は毎秒25回から2回に低下します" },
            ["关闭过渡动画：歌词切换的淡入淡出与位移、窗口位置与尺寸动画、模块出现与消失动画、悬停光晕均改为即时切换"] = new[] { "关闭过渡动画：歌词切换的淡入淡出与位移、窗口位置与尺寸动画、模块出现与消失动画、悬停光晕均改为即时切换", "關閉過渡動畫：歌詞切換的淡入淡出與位移、視窗位置與尺寸動畫、模組出現與消失動畫、懸停光暈均改為即時切換", "Disables transitions: lyrics fade and slide, window position and size animations, module enter and exit animations, and the hover aura all switch instantly", "トランジションを無効化：歌詞のフェードと移動、ウィンドウ位置とサイズのアニメーション、モジュールの出現と消失、ホバーオーラはすべて即時切り替えになります" },
            ["开启后的影响：歌词岛没有平滑过渡，悬停探测响应略迟钝，但功耗显著降低"] = new[] { "开启后的影响：歌词岛没有平滑过渡，悬停探测响应略迟钝，但功耗显著降低", "開啟後的影響：歌詞島沒有平滑過渡，懸停偵測回應略遲鈍，但功耗顯著降低", "Effects when enabled: no smooth transitions on the lyrics island and slightly slower hover detection, but power usage drops significantly", "有効化後の影響：歌詞アイランドに滑らかなトランジションがなくなり、ホバー検知の応答もわずかに遅れますが、消費電力は大幅に下がります" },
