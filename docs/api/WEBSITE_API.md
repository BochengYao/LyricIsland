...
公开页和管理端只提供“本版本”与“稍后推出”两种展示层级：历史 `featured` 与 `improvement` 均作为本版本功能按 `sort_order` 合并展示，新建本版本功能写入 `featured`；`improvement` 继续作为兼容值由 API 接受和原样保存，不强制改写旧数据。只有 `future` 独立分组，并按 `target_version`、`sort_order` 展示。
