...
- 旧 `body + highlights` 安全适配为 Note + Features；缺失 progress 读取为 0% 未开始。无法判断 Note 的旧长文本不自动挪动，只有需求指定的 V3.2 中文首条总说明采用精确匹配迁移；旧进度缺失读取为 0% 未开始；旧 100% 记录默认按待测试处理。
- 后台保留批量中文粘贴解析，并提供稳定 ID、四语逐项内容、排序、删除、新增、离散锚点状态选择；不引入拖拽库。滑块共 10 档：0、10、30、50、65、80、90、95、测试中、待上线；后两档分别保存为 `stage=testing` 与 `stage=ready`、`progress=100`。
- 翻译使用 `note` / `feature.<id>` 键并严格校验完整键集合；提供“翻译全部”和“仅翻译缺失内容”。
- 公开页保持原双栏与留白，左侧增加 Note，右侧编号替换为 16px SVG 圆环；不显示百分比、状态标签或横向进度条。四种状态为灰色 0%、橙黄开发中进度环、测试中单个橙黄呼吸环、待上线单个橙黄纯色环；前台两处版本预告入口均显示四语图例。
...
- 后台进度编辑使用一条 10 档滑块：0、10、30、50、65、80、90、95、测试中、待上线，不接受任意百分比。
- `ReleasePreviewFeature.stage` 保存 `development` / `testing` / `ready`；后两者强制要求 100%。旧 null 读取为 0% 未开始；历史任意百分比向下归入相邻开发锚点，历史 100% 默认按测试中读取。
- 圆环状态：0% 全灰；10–95% 橙黄进度环；测试中为单个橙黄纯色圆环，以 1.05 秒透明度呼吸明显闪烁且不使用发光阴影；待上线为单个橙黄纯色静态环。前台 updates 与 incentives 均保留四语图例。
- 后台每条功能改为紧凑编辑布局：标题与排序操作共用一行，语言标签与内容框在桌面端横排，短内容框约两行高，长内容按实际内容自动增高；进度标题、滑块、锚点与当前状态横向组合。移动端恢复单列，内容不截断。
- “测试中”的透明度呼吸动画作用于固定 16×16 SVG 外框，不再作用于带旋转属性的 circle；图例内四个图标统一清除顶部偏移并与文字垂直居中。
