...
- 后台保留批量中文粘贴解析，并提供稳定 ID、四语逐项内容、排序、删除、新增、离散锚点状态选择；不引入拖拽库。滑块 100% 表示开发完成待测试，继续向右分别进入 `stage=testing` 与 `stage=ready`。
...
- 后台进度编辑改为一条滑块：0–100 为开发进度，继续向右依次为“开发完成，待测试”和“测试完成，待上线”。
- `ReleasePreviewFeature.stage` 保存 `development` / `testing` / `ready`；后两者强制要求 100%。旧 null 或历史非法值读取为 0% 未开始。
