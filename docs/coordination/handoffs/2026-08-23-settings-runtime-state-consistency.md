...
- UI 临时状态：设置窗口只接收 `CreateEditSnapshot()` 的 detached draft。当前集成 UI 采用 220ms 防抖自动 Apply，防抖期间是唯一未提交生命周期；关闭窗口前会提交待处理修改，因此不会遗留草稿。旧 Apply/Cancel 控件保持隐藏，不作为业务状态来源。
- Apply：显式 Apply、教程提交或防抖自动 Apply 均走同一个协调器边界。协调器克隆并规范化 draft，调用 MainWindow 的运行时同步，再保存运行时修正后的 effective settings，并把该快照返回给 UI。LyricDock 强制关闭等回落会同时反映到文件、运行时和开关。
...
- 窗口生命周期：设置窗口不再用打开/关闭生命周期长期暂停鼠标避让；临时交互抑制由 MainWindow 的运行时路径管理。窗口关闭会停止防抖与预览资源，并保证最后一次有效状态已通过协调器提交。
...
- `LyricHover.App/PlacementSettingsWindow.xaml.cs`：Apply 接收 effective snapshot；防抖自动提交与关闭收尾共用协调器边界；外部 LyricDock 禁用与 UI 状态保持一致。
...
- 定向测试：`dotnet run --project LyricHover.Tests -c Release --no-build -- --settings-runtime-state-fixture`，退出码 `0`，4/4 PASS：
  - detached draft 隔离：Apply 前 committed 与文件均不变。
...
- 集成后完整桌面测试：退出码 `0`，全部 PASS；新增 4 项设置状态测试全部 PASS。同步迁移了根设置 UI WIP 已改变的静态契约，并恢复 Support 自由操作区的自适应垂直布局。
- 正式发布脚本：`tools/publish-next-version.ps1 -NoLaunch`，完整测试、win-x64 Release 构建和 publish 全部通过，输出 `发布完成：v3.1.31 Beta`。
- 产物核验：`publish/current/LyricHover.App.dll` ProductVersion=`3.1.31-Beta`、FileVersion=`3.1.31.0`、SHA-256=`1F22444BB96435090189CE7094D2BABFBB38850426C08F484317824F4E244FF0`；旧 `v3.1.30-Beta` 已归档。
- `git diff --check`：通过。
...
7. 鼠标避让暂停属于 MainWindow 的瞬态运行时状态；设置页不得通过隐藏控件或窗口可见性直接控制底层避让业务。

## 同步需求

- Desktop Core：不需要 Core 代码同步；需要由桌面契约维护者知悉新增可选持久化键及默认/缺失值兼容行为。
- Desktop UI & Interaction：已同步。`6ffe637` 已在当前设置页 WIP 上完成语义集成，最终提交为 `a92558b`；仍需人工审查真实 WPF 交互。
...
- 已知限制：自动测试覆盖保存、草稿隔离、重载和窗口生命周期；真实 WPF 窗口的点击、自动 Apply、关闭与重启恢复仍需人工验收。
